...
class _TodoPageState extends State<TodoPage> {
  final GlobalKey<ScaffoldState> _scaffoldKey = GlobalKey<ScaffoldState>(); // Drawer 操作用キー
  ...
  Future<void> _renameCategory() async {
    ...
    final newName = await showDialog<String>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(onPressed: () => Navigator.pop(ctx, nameController.text.trim()), child: const Text('変更')),
        ],
      ),
    );
    ...
    if (newName != null && newName.isNotEmpty && newName != oldName) {
      final dir = await getApplicationDocumentsDirectory();
      final oldFile = File('${dir.path}/todos_${oldName.replaceAll(' ', '_')}.json');
      final newFile = File('${dir.path}/todos_${newName.replaceAll(' ', '_')}.json');
  ...
  Future<void> _createNewCategory() async {
    ...
    final name = await showDialog<String>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, null), child: const Text('キャンセル')),
          TextButton(onPressed: () => Navigator.pop(ctx, nameController.text.trim()), child: const Text('作成')),
        ],
      ),
    );
  ...
  Future<void> _pickDateRange() async {
    ...
    final time1 = await showTimePicker(context: context, initialTime: TimeOfDay.now());
    if (time1 == null) return;
    final date2 = await showDatePicker(
        context: context, firstDate: date1, lastDate: DateTime(2100), initialDate: date1);
    if (date2 == null) return;
    final time2 = await showTimePicker(
        context: context, initialTime: TimeOfDay(hour: time1.hour, minute: time1.minute));
    if (time2 == null) return;

    setState(() {
      _pickedStart = DateTime(date1.year, date1.month, date1.day, time1.hour, time1.minute);
      _pickedEnd   = DateTime(date2.year, date2.month, date2.day, time2.hour, time2.minute);
    });
  }
  ...
  Widget _buildInputRow() { // double sw を削除
    return Row(
      children: [
        Expanded(
          child: TextField(
        ...
        ElevatedButton(
          ...
          style: ElevatedButton.styleFrom(
            ...
            padding: const EdgeInsets.symmetric(vertical: 14, horizontal: 20), // パディングを調整
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
            elevation: 8,
            shadowColor: Colors.cyanAccent.withOpacity(0.4),
          ),
          child: const Text('追加', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        ),
      ],
    );
  }

  Widget _buildTodoList() { // double sw, double sh を削除
    if (_todos.isEmpty) {
      return const Expanded(child: Center(child: Text('Todoがありません', style: TextStyle(fontSize: 18, color: Colors.white70)))); // スタイルを調整
    }
    return Expanded(
          ...
          child: ListTile(
            ...
            title: Column(
              ...
              children: [
                ...
                if (_todos[i].start != null && _todos[i].end != null) // 日時表示を追加
                  Text(
      ...
      onPressed: () {
        ...
        for (var i = 0; i < _todos.length; i++) {
          ...
          if (t.start != null) {
            ...
            events.putIfAbsent(key, () => []).add(Event(t.text, Colors.primaries[i % Colors.primaries.length]));
          }
        }
        Navigator.push(context, MaterialPageRoute(builder: (_) => CalendarPage(events: events)));
      },
    );
  }

  Widget _buildDrawerButton() { // Drawerを開くFABを追加
     return FloatingActionButton(
       heroTag: 'openDrawer',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('ToDoリスト', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
      ...
      body: Stack(
        children: [
          ...
          SafeArea(
            child: Padding(
              padding: const EdgeInsets.symmetric(horizontal: 18, vertical: 24), // SingleChildScrollViewはColumn内に移動
              child: Column(
              ...
              children: [
                ...
                GlassCard(
                  child: DrawerHeader(
                    ...
                    child: const Text('リストメニュー', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
                  ),
                ),
                ...
                GlassCard(
                  child: ListTile(
                    ...
                    title: const Text('新規リスト作成', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w500)),
                    onTap: () {
                      Navigator.pop(context);
                      _createNewCategory();
                    },
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (ctx, i) {
                      ...
                      return GlassCard(
                        child: ListTile(
                          title: Text(name, style: TextStyle(color: name == _currentCategory ? Colors.cyanAccent : Colors.white, fontWeight: name == _currentCategory ? FontWeight.bold : FontWeight.normal)),
                          ...
                          selectedTileColor: Colors.white.withOpacity(0.08), // 選択時の背景色を調整
                        ),
                      );
                    },
                  ),
      ...
      floatingActionButtonLocation: FloatingActionButtonLocation.miniStartFloat, // FABの位置調整（Stackを使う場合はあまり影響しないかも）
    );
  }
}
